...
export function SetHandleWidth(value) {
	controlHandles.children.forEach((h) => h.scale.setScalar(value));
}
...
export function InitialiseControlHandles(scene, inputs) {
	...
	for (let point of startPoints) {
		...
		if (isNaN(inputs)) {
			circle.scale.setScalar(inputs.handles.size.value);
		} else {
			circle.scale.setScalar(parseInt(inputs));
		}
...
export function AddControlPoint(scene, inputs) {
	...
	const midpoint = new THREE.Vector3(
		(controlHandles.children[controlHandles.children.length - 2].position.x +
			controlHandles.children[controlHandles.children.length - 1].position.x) /
			2,
		(controlHandles.children[controlHandles.children.length - 2].position.y +
			controlHandles.children[controlHandles.children.length - 1].position.y) /
			2,
	...
	handle.scale.setScalar(inputs.handles.size.value);
...
export function RemoveControlPoint() {
	...
	controlHandles.remove(controlHandles.children[controlHandles.children.length - 1]);
	console.log(controlHandles);
}
